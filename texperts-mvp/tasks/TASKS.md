...
## Epic 11: Remaining Tasks (Backlog)
> 15 tasks deferred from Milestones 3–8. Complete these to reach 100% coverage.

| # | Task | Origin | Why not done |
|---|---|---|---|
11.1.1 | 3.4.3 | Bubble overlap prevention between nearby agents | M3 Bubbles | No offset logic for nearby agents |
11.2.1 | 5.1.6 | Rate limiting (500ms between agent calls) | M5 Claude | Only a sleep delay, not proper rate limiting |
11.3.1 | 5.4.3 | Test single agent: inject scenario → agent plans → acts → speaks | M5 Claude | Manual testing task, not code |
11.4.1 | 6.1.1 | LLM-based importance scoring on memory creation | M6 Cognitive | `scoreImportance()` exists but isn't auto-called; importance is hardcoded per action type |
11.5.1 | 6.3.1 | Plan decomposition (high-level plan → sub-actions) | M6 Cognitive | `Planner.ts` is a stub returning empty |
11.6.1 | 6.3.2 | Action sequences: move_to → speak → wait → move_to | M6 Cognitive | Stub |
11.7.1 | 6.3.3 | Re-evaluate plan when significant new perceptions arrive | M6 Cognitive | Stub |
11.8.1 | 6.6.3 | Emit phase_change events via SSE | M6 Cognitive | PhaseManager exists but isn't wired into Orchestrator tick loop |
11.9.1 | 7.2.5 | Show relationship summary (opinions of other agents) | M7 UI | Not tracked or displayed |
11.10.1 | 7.2.6 | Show full conversation log for this agent | M7 UI | Not displayed in inspector |
11.11.1 | 7.3.2 | Event log filter by type | M7 UI | No filter UI |
11.12.1 | 7.3.3 | Event log filter by agent | M7 UI | No filter UI |
11.13.1 | 8.4.3 | "Follow active speaker" toggle | M8 Polish | Camera follows selected agent only, no speaker-tracking toggle |
11.14.1 | 8.5.2 | Edit agent names, roles, personas, colors in setup | M8 Polish | Setup shows agents but fields aren't editable |
11.15.1 | 8.5.3 | Edit scenario briefing text in setup | M8 Polish | Setup shows briefing but isn't editable |
...
|-----------|---------|-------|------|-----------|
| 1. The World | 7 | 22 | 22 | 0 |
| 2. Agents on Map | 4 | 14 | 14 | 0 |
| 3. Bubbles | 5 | 14 | 13 | 1 |
| 4. Backend | 5 | 18 | 18 | 0 |
| 5. Claude | 6 | 20 | 18 | 2 |
| 6. Cognitive Loop | 6 | 18 | 14 | 4 |
| 7. User Interaction | 6 | 16 | 12 | 4 |
| 8. Polish | 6 | 13 | 9 | 4 |
| **Total** | **45** | **135** | **120** | **15** |
